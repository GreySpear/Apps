/*
 * Kitchen — service worker for offline support.
 *
 * Goal: the app shell (this single-file web app + its fonts) loads with NO
 * connection, so you can open your recipes, plan, and grocery list on the
 * subway. Your data already lives in localStorage and syncs to the Google
 ...
 *     the network so data stays live; POSTs (writes) are ignored by the fetch
 *     handler entirely. When offline these fail and the app falls back to its
 *     localStorage cache on its own.
 *
